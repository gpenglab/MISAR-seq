...
refine_expression_kmean <- function(pred, num_nbs = 4){
  res <- map(unique(pred$Sample), ~{
    sample_id = pred[pred$Sample == .x,]$cellNames
    x_array = pred[pred$Sample == .x,]$array_row
    y_array = pred[pred$Sample == .x,]$array_col
    expression = pred[pred$Sample == .x,]$expression
    adj_2d <- calculate_adj_matrix(x=x_array,y=y_array)
    
    y_pred <- kmeans(expression, 3)
    
    refined_pred = refine_modify(sample_id=sample_id, pred=y_pred$cluster, dis=adj_2d, shape="square")
    
    y_pred_mean <- data.frame(label = y_pred$cluster, expression = expression) %>% dplyr::group_by(label) %>% dplyr::summarize(mean = mean(expression, na.rm = T))
    
    idx <- unlist(map2(y_pred$cluster, refined_pred, ~{
      .x != .y
    }))
    
    expression[idx] <- y_pred_mean[as.numeric(refined_pred[idx]),]$mean
    names(expression) <- sample_id
    return(expression)
  }) %>% Reduce(c,.)
  return(res)
}
...
#Same as addImputeWeights in ArchR but saved impute matrix.
#Redefine this function because we can't get impute matrix after using addImputeWeights. 
Impute_MAGIC <- function(
    mat,
    matDR, 
    td = 3,
    ka = 4,
  ...
  weightList <- ArchR:::.safelapply(seq_len(nRep), function(y){
    ...
    blockList <- lapply(seq_along(blocks), function(x){
      ...
      if(x %% 10 == 0){
        sprintf("Computing Partial Diffusion Matrix with Magic (%s of %s, Iteration %s of %s)", y, nRep, x, length(blocks))
      }
  ...
  results.ls <- list()
  results.ls$imputeMat <- imputeMat
  results.ls$weightList <- weightList
  return(results.ls)
}
