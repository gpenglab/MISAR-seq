...
#replace arrow file if needed
#proj@sampleColData@listData[["ArrowFiles"]] <- "./E15_5-S1/E15_5-S1.arrow"
...
# using spatial position or histology (pixel intensity + position) to smooth
# for ATAC
proj <- IterativeLSI(
  ArchRProj = proj,
...
# using Impute_MAGIC from function.R to remove noise
#smooth RNA by MAGIC
df_rna <- getMatrixFromProject(proj,useMatrix = "GeneExpressionMatrix")
dat_rna <- df_rna@assays@data@listData[["GeneExpressionMatrix"]]
rownames(dat_rna) <- df_rna@elementMetadata@listData[["name"]]

impute_rna <- Impute_MAGIC(
  mat = as.matrix(dat_rna),
  matDR = proj@reducedDims@listData[["LSI_RNA"]]@listData[["matSVD"]],
  threads = 30)

rna <- df_rna@assays@data@listData$scale.data <- vegan::decostand(impute_rna$imputeMat, "range", 1)

#smooth ATAC by MAGIC
df_atac <- getMatrixFromProject(proj,useMatrix = "GeneScoreMatrix")
dat_atac <- df_atac@assays@data@listData[["GeneScoreMatrix"]]
rownames(dat_atac) <- df_atac@elementMetadata@listData[["name"]]

impute_atac <- Impute_MAGIC(
  mat = as.matrix(dat_atac),
  matDR = proj@reducedDims@listData[["LSI_ATAC"]]@listData[["matSVD"]],
  threads = 30)

atac <- df_atac@assays@data@listData$scale.data <- vegan::decostand(impute_atac$imputeMat, "range", 1)

#Plot Imputed Spatial marker gene of Expression and Gene_Score
marker_gene <- "Neurod1"

dat <- cbind(Expression = rna[marker_gene,], Gene_Score = atac[marker_gene,], as.data.frame(proj@cellColData))

# plot spatial Expression and Gene_Score of marker gene
pdf(file = paste0(marker_gene, "_" ,sampleId, "_Spatial_Expression_Gene_Score.pdf"), width=8.6, height=8.6)

for(i in c("Expression","Gene_Score")){
  proj <- addCellColData(ArchRProj = proj, data = dat[,i], cells = rownames(dat), name = "expression", force = T)
  
  # using refine_expression_kmean from function.R to smooth
  proj$expression <- refine_expression_kmean(proj)
  
  df <- as.data.frame(proj@cellColData)
  
  p6 <- ggplot(df, aes(x = as.numeric(array_col), y = as.numeric(array_row), color=expression)) + 
    ggtitle(paste0(sampleId,"_",marker_gene,"_",i)) +
    # scale_color_manual(values = color_list) +
    annotation_custom(g, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) +
    geom_point(size = 2.5,shape=15) +
    scale_color_gradientn(colours = ArchRPalettes$blueYellow) +
    expand_limits(x = 0, y = 0) +
...
saveRDS(proj, paste0(sampleId,".rds"))
